...
import Style from "./TodaysSpecialStyle";
...
export default class TodaysSpecial extends Base {
  static instance;
  ...
  componentDidMount() {
    ...
    this.props.navigation.setParams({ flag: false });

    SideMenu.fetchDetails();
    this.callBack();
    if (this.props.navigation.state.params.title == "Fresh List") {
      Service.getInstance()._getWishlist(this, this._getCategorySuccess, this._onFailure);
    } else {
      Service.getInstance()._getCategoryById(this.props.navigation.state.params.id, this, this._getCategorySuccess, this._onFailure);
    }
  }

  // static callBasket(){
  //   instance.props.navigation.navigate("Basket", { callBack: this.callBack });
  // }
  ...
  static searchMethod(flag) {
    ...
    instance.props.navigation.setParams({ flag: flag });
  }
      ...
      <View style={Style.root}>
        ...
        />
        {
          this.state.searchFlag ?
            <SearchBar
              ...
              inputStyle={{ backgroundColor: Color.white, color: Color.darkgree }}
        ...
        {this.state.noFlag ?
          <View style={Style.noOrder} >
            <Text style={[Style.pricespl, { fontSize: 20 }]}> Add your {this.props.navigation.state.params.title}</Text></View> :
          <View style={{ flex: 1 }}>
            <FlatList
